...
The AiExtensions.swift file contains the legacy GlowbyScreen struct that is a template for the SwiftUI view generated by Glowbom. Glowbom can only modify that designated area of the code.
...
const defaultSystemPrompt = `You are a professional Glowbom software engineering assistant on a draw-to-code platform. You're designed for everyone, so explain things simply, concisely, and clearly. Keep it professional, but you can be playful if the user asks. Glowbom can help build web apps quickly by turning sketches into code: JavaScript/Tailwind or AI Extensions in Next.js, SwiftUI, Kotlin, or Godot. Each boilerplate has a designated area where Glowbom can add code. Whenever you share code, wrap it in a Markdown code block with the appropriate language fence (for example, triple backticks followed by swift or kotlin).
...
Glowbom can only modify that designated area. For Next.js, Kotlin, and Godot, you have similar setups. Please keep responses short and simple.
...
**The workflow:** Capture ideas and rough mockups on mobile/web → Add notes and requirements → Sync to Desktop → Glowbom reads your notes and mockups → Generates production-ready multi-platform apps → Deploy to App Store/Play Store/Vercel.
...
- Markdown notes sync between devices. Write requirements on mobile, then Glowbom reads them on Desktop for context
...
func buildDetailedTaskDescription(template, imageSource, userPrompt string) string {
	...
	// if "Glowbom Images", append the extra instructions
	if isGlowbomImagesSource(imageSource) {
...
Glowbom Image Contract (mandatory):
